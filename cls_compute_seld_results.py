...
class ComputeSELDResults(object):
    def __init__(self, params, ref_files_folder=None):
        ...
        # [수정] os.walk를 사용하여 하위 폴더 구조에 상관없이 모든 csv 파일 탐색
        for root, dirs, files in os.walk(self._desc_dir):
            for ref_file in files:
                if not ref_file.endswith('.csv'):
                    continue
                    
                # Load reference description file
                full_path = os.path.join(root, ref_file)
                gt_dict = self._feat_cls.load_output_format_file(full_path, cm2m=True)
