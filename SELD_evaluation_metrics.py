...
def _safe_nanmean(x, default=0.0):
    arr = np.asarray(x)
    if arr.size == 0:
        return default
    if np.isnan(arr).all():
...
class SELDMetrics(object):
    ...
    def compute_seld_scores(self):
        ...
        if self._average == 'micro':
            ...
            F       = _safe_mean(F, default=0.0)
            AngE    = _safe_nanmean(AngE, default=np.nan)    # 각도는 표시상 NaN 허용
            DistE   = _safe_nanmean(DistE, default=np.nan)
            RelDistE= _safe_nanmean(RelDistE, default=np.nan)
            LR      = _safe_mean(LR, default=0.0)
            SELD_scr= _safe_mean(SELD_scr, default=1.0)      # 비어있으면 최악점수 1.0
            IDSR    = _safe_nanmean(IDSR, default=np.nan)
